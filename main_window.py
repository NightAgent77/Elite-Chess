...
from PyQt5.QtCore import Qt, QRectF, pyqtSignal  # Ensure QRectF and pyqtSignal are imported
...
class BorderlessWindow(QMainWindow):
    def __init__(self):
        ...
        # Add a flag to track the current layout
        self.current_layout = "main"

        self.init_ui()

        # Ensure the application always starts in the main layout
        self.current_layout = "main"
        self.theme_left_widget.setVisible(False)
    ...
    def init_widgets(self):
        ...
        self.theme_icon_widget.clicked.connect(self.on_theme_icon_clicked)  # Connect the clicked signal
        ...
        self.chess_game_icon_widget.clicked.connect(self.on_chess_game_icon_clicked)  # Connect the clicked signal
        self.adjust_chess_game_icon_widget()

        # Initialize and add the ThemeLeftWidget
        self.theme_left_widget = ThemeLeftWidget(self)
        self.adjust_theme_left_widget()
    ...
    def adjust_theme_left_widget(self):
        # Adjust the ThemeLeftWidget position and size to be right next to the LeftWidget
        self.theme_left_widget.setGeometry(
            self.left_widget_width,  # Position it immediately to the right of the LeftWidget
            0,  # Align to the top
            self.left_widget_width,  # Same width as the LeftWidget
            self.height()  # Full height of the window
        )
    ...
    def on_theme_icon_clicked(self):
        logging.info("Theme icon clicked! Switching to theme layout.")

        # Update the current layout flag
        self.current_layout = "theme"

        # Change the theme icon to red to indicate the current layout
        self.theme_icon_widget.icon_label.setPixmap(self.theme_icon_widget.hover_icon)

        # Ensure the chessboard widget is removed when switching to the theme layout
        if hasattr(self, 'chessboard_widget'):
            self.chessboard_widget.setParent(None)
            del self.chessboard_widget

        # Add a placeholder or theme-specific content
        self.theme_placeholder = QLabel("Theme Layout", self)
        self.theme_placeholder.setStyleSheet("color: white; font-size: 24px;")
        self.theme_placeholder.setGeometry(
            self.left_widget_width + 50,  # Adjust position to the right of the left widget
            50,  # Adjust vertical position
            self.width() - self.left_widget_width - 100,  # Adjust width
            self.height() - 100  # Adjust height
        )
        self.theme_placeholder.setAlignment(Qt.AlignCenter)

        # Show the ThemeLeftWidget only in the theme layout
        self.theme_left_widget.setVisible(self.current_layout == "theme")

    def on_chess_game_icon_clicked(self):
        logging.info("Chess game icon clicked! Switching back to main window layout.")

        # Update the current layout flag
        self.current_layout = "main"

        # Change the theme icon back to white to indicate the current layout
        self.theme_icon_widget.icon_label.setPixmap(self.theme_icon_widget.default_icon)

        # Remove the theme placeholder if it exists
        if hasattr(self, 'theme_placeholder'):
            self.theme_placeholder.setParent(None)
            del self.theme_placeholder

        # Recreate the chessboard widget
        self.chessboard_widget = ChessBoardWidget(self)
        self.adjust_chessboard_widget()

        # Ensure the chessboard widget is visible
        self.chessboard_widget.show()
...
class ThemeLeftWidget(QWidget):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.init_ui()

    def init_ui(self):
        # Set the background color to a slightly darker grey for the ThemeLeftWidget
        palette = QPalette()
        palette.setColor(QPalette.Window, QColor(44, 44, 44))  # RGB for darker grey
        self.setPalette(palette)
        self.setAutoFillBackground(True)
...
class ThemeIconWidget(QWidget):
    clicked = pyqtSignal()  # Signal to notify when the widget is clicked
    ...
    def enterEvent(self, event):
        # Change to hover icon only if not in the theme layout
        if self.parent().parent().current_layout != "theme":
            self.icon_label.setPixmap(self.hover_icon)
        super().enterEvent(event)

    def leaveEvent(self, event):
        # Revert to default icon only if not in the theme layout
        if self.parent().parent().current_layout != "theme":
            self.icon_label.setPixmap(self.default_icon)
...
class ChessGameIconWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.icon_label = QLabel(self)
        self.default_icon = QPixmap("src/assets/Play Icons/white_chess_game.png")
        self.hover_icon = QPixmap("src/assets/Play Icons/red_chess_game.png")
        self.icon_label.setPixmap(self.default_icon)
        self.icon_label.setScaledContents(True)
